...
public class Doctor : GAgent
{
    ...
    {
        base.Start();
        SubGoal s1 = new SubGoal("Research", 1, false); //goal, priority, boolean. This will remove it from being a goal once its true. false keeps it repeating.
        ...
        SubGoal s3 = new SubGoal("TakeRestroomBreak", 1, false); //goal is to use restroom, priority of 1 and do not remove it as a goal, let it repeat.
        goals.Add(s3, 3);

        Invoke("GetTired", Random.Range(10.0f,20.0f));//Invoke the GetTired() from the start.
